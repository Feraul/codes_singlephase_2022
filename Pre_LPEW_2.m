...
for No=1:size(coord,1)
    ...
    [ neta,gaux] = netas_Interp_LPEW( O, P, T, Qo, No,kmap,gradgravelem,flagbedge);
   ...
   %    theta2, ve1, ve2, neta, P, O,r,gauxm);
    ...
    if strcmp(gravitational,'yes')
        ...
        %wg(No,1)=(sum(gaux3))/sum(lambda); % erro -> 0.0038
    end
    ...
    if comp1<= size(bedge,1) && comp2 <=size(bedge,1) && 200<bedge(MMM,4)
        ...
        if strcmp(gravitational,'yes')
           ...
           R=[0 1; -1 0 ];
            ...
            v2=coord(No,1:2)-0.5*(coord(bedge(comp2,2),1:2)+coord(bedge(comp2,1),1:2));
